...
/**
 * Render data of a single painted cursor, previously exported by the removed
 * DOM view-cursors renderer. The Go engine owns cursor painting, so the array
 * is always empty in GodeView; the type is kept for the hit-testing plumbing.
 */
...
/** CSS class of the (removed) DOM view-line container. */
const VIEW_LINE_CLASS_NAME = 'view-line';
...
export class MouseTargetFactory {
	...
	private static _actualDoHitTestWithCaretRangeFromPoint(ctx: HitTestContext, coords: ClientCoordinates): HitTestResult {
		...
		if (startContainer.nodeType === startContainer.TEXT_NODE) {
			...
			if (parent3ClassName === VIEW_LINE_CLASS_NAME) {
			...
			if (parent2ClassName === VIEW_LINE_CLASS_NAME) {
