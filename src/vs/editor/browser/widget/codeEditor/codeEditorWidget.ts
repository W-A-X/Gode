...
import { IThemeService, registerThemingParticipant, IColorTheme } from '../../../../platform/theme/common/themeService.js';
...
export interface IGodeViewFactoryArgs {
	...
	readonly userInteractionService: IUserInteractionService;
}

export type GodeViewFactory = (args: IGodeViewFactoryArgs) => View;

let godeViewFactory: GodeViewFactory | undefined;

/**
 * Registers a factory that replaces the default DOM-based editor view with a
 * custom rendering backend (e.g. the gogpu/ui-based gode-engine process).
 * Called by the workbench contribution when the feature is enabled.
 */
export function setGodeViewFactory(factory: GodeViewFactory | undefined): void {
	godeViewFactory = factory;
}
...
export class CodeEditorWidget extends Disposable implements editorBrowser.ICodeEditor {
	...
	protected _createView(viewModel: ViewModel): [View, boolean] {
		...
		if (godeViewFactory) {
			return [godeViewFactory({
				editorContainer: this._domElement,
				ownerID: this.getId(),
				commandDelegate,
				...
				userInteractionService: this._userInteractionService,
			}), true];
		}
