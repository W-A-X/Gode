...
func (e *Engine) SetTabs(tabs []TabInfo, activeIdx int) {
        ...
        e.tabBar.Update(editorTabs, activeIdx)
        e.tabsDirty = true
}

// SetTabViewport sets the rendering dimensions for the tab bar.
func (e *Engine) SetTabViewport(w, h int, scale float32) {
        if scale <= 0 {
                scale = 1
        }
        if w == e.tabWidth && h == e.tabHeight && scale == e.tabScale {
                return
        }
        e.tabWidth, e.tabHeight, e.tabScale = w, h, scale

        // Rebuild tab render target
        e.tabCC = gg.NewContext(w, h)
...
// RenderTabBar renders the tab bar and returns RGBA pixels. Returns nil if no
// update is needed since last render.
func (e *Engine) RenderTabBar() ([]byte, bool) {
        if !e.tabsDirty {
                return nil, false
        }
...
// HandleTabEvent dispatches a mouse event to the tab bar. Returns true if handled.
func (e *Engine) HandleTabEvent(me InputMouse) bool {
        ev := BuildMouseEvent(me)
        return e.tabBar.HandleEvent(ev)
}

// TabViewportSize returns the current tab bar dimensions.
func (e *Engine) TabViewportSize() (int, int) {
